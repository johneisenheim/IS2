...
public class SelezionaFornitore extends JPanel {
	...
	private SelezionaFornitore pannelloCorrente = null;
			...
			jButtonVsualizza.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if (jTable.getSelectedRow()>=0) 
						new ModificaVisualizzazioneF( (String) jTable.getValueAt(jTable.getSelectedRow(), 0),pannelloCorrente);
				}
			});
		}
	...
	private JComboBox getJComboBoxTipologia() {
		if (jComboBoxTipologia == null) {
			...
			jComboBoxTipologia.addItem("PIva");
			jComboBoxTipologia.addItem("RagSociale");
			jComboBoxTipologia.addItem("Fiducia");
		}
		...
		if (jButtonCancella == null) {
			...
			jButtonCancella.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (conferma == 0){ 
					GestioneFornitore a=new GestioneFornitore();
					Fornitore toDelete = a.getFornitore( (String) jTable.getValueAt(jTable.getSelectedRow(),0) );
					a.CancellaFornitore(toDelete);
					aggiorna_tabella();
					}
				}
			});
	...
	public JTable getJTable() {
		if (jTable == null) {
			GestioneFornitore b = new GestioneFornitore();
			String list[][] = b.selezionaFornitore();
			String []columnNames = {"PIva","RagSociale","Fiducia","PagPredefinito","Rappresentante"};
			jTable = new JTable(list,columnNames);
	...
	public void aggiorna_tabella(){
		GestioneFornitore a=new GestioneFornitore();
		String lista[][] =a.selezionaFornitore();
		String []columnNames = {"PIva","RagSociale","Fiducia","PagPredefinito","Rappresentante"};
